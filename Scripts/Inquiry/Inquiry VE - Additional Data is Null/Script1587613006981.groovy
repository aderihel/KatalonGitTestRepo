...
GlobalVariable.reqContType = 'application/json'

GlobalVariable.reqBody = (((('{"client": "BRIMON", "request_refnum": "' + CustomKeywords.'database.methods.updateRefNum'()) + 
'", "username": "bribri0001", "purchase_number": "087705361213", "purchase_id": "15", "timestamp": "') + unixTime) + '", "channel_id": "NBMB"}')

WS.comment(unixTime)

response = WS.sendRequest(findTestObject('Inquiry/Inquiry Empty', [('Body') : GlobalVariable.reqBody]))
...
WS.comment('#18 Verify Verify if Response Code is not null')

WS.verifyNotMatch(response_code, null, false)

WS.comment('#20 Verify Verify if Response Id is not null')

WS.verifyNotMatch(response_id, null, false)

WS.comment('#22 Verify Verify if Response Refnum is not null')

WS.verifyNotMatch(response_refnum, null, false)

WS.comment('#24 Verify Verify if Response Desc is not null')

WS.verifyNotMatch(response_desc, null, false)

WS.comment('#26 Verify Verify if Response Data is not null')

WS.verifyNotMatch(response_data, null, false)

WS.comment('#28 Verify Verify if Response Code Length is 2')

WS.verifyMatch(code_length, '2', false)

WS.comment('#30 Verify Verify if Response Refnum Length is 12')

WS.verifyMatch(refnum_length, '12', false)

WS.comment('#32 Verify Verify if Response Code is VE')
